/*
     ...
     https://github.com/Tech500/Furnace-Monitor/blob/master/Furnace_Monitor.ino
...
String lastUpdate;   //Holds last update Date Time Stamp.
...
int count = 0;  //used in testing only.

int days;   //Holds number of days in Month
//int Month;  

/*
     Graphing is setup to use a "Thingspeak.com" free account.  This is the ThingSpeak channel number 
     for the MathwWorks weather station 
...
float event = 0;;        //elapsed time in milliseconds for day. 
float totalDay = 0;      //elapsed time in minutes
float total_Day = 0;     //elapsed time in milliseconds for day.
float totalMonth = 0;    //elapsed time in minutes
float total_Month = 0;   //elapsed time in milliseconds for day.
float totalYear = 0;     //elapsed time in minutes
float total_Year = 0;    //elapsed time in milliseconds for day.

String tFD;
String tFY;

//////////////////////////////
void ICACHE_RAM_ATTR runTime()
{
...
void loop()
{
	   ...
	   if (off_Flag )
     {
          ...
          Serial.println("");
          event = elapsed;
          total_Day = total_Day + event;
	      	totalDay = total_Day / 60000;  //Convert from milliseconds to minutes
          total_Month = total_Month + event;
	      	totalMonth = total_Month / 60000;  //Convert from milliseconds to minutes
          total_Year = total_Year + event;
	      	totalYear = total_Year / 60000;  //Convert from milliseconds to minutes
          Serial.print("elapsed:  ");
          Serial.println(event);
          Serial.print("totalDay:  ");
          Serial.println(totalDay, 4);
          Serial.print("totalMonth:  ");
          Serial.println(totalMonth, 4);
          Serial.print("totalYear:  ");
          Serial.println(totalYear, 4);
          Serial.println("");
          Serial.println("");
          Serial.println(dtStamp);
          elapsed = 0;
...
//if((HOUR == 0) && (MINUTE == 0) && (SECOND == 0))
{
     ...
     Serial.print(totalDay, 4);
     Serial.println("  ");
     Serial.print("  Total Minutes for Month");
     Serial.print(":  ");
     Serial.print(totalMonth, 4);
     Serial.println("");
     Serial.print("  Total Minutes Year:  ");
     Serial.println(totalYear, 4);  //change variable name to yearAccumlating
     Serial.println(dtStamp);
     Serial.println("");
     ...
     if (count == 10) //testing
     //if (MONTH == 12) && (DATE == days)  //End of year
     {
...
////////////////////////
void writemonthEnd()  //write --totals for month in milliseconds.
{

     File logFile = SPIFFS.open("/MONTHEND.TXT", "a");

     if (!logFile)
     {
          Serial.println("  File MONTHEND.TXT failed to open");
          Serial.println("");

     }
     else
     {
          String monthData;   //Create String data = (eoealMonth, dtStamp)
          monthData = "Month end elapsed time:  ";
          monthData += totalMonth;
          monthData += " , ";
          monthData += dtStamp;

          logFile.println(monthData);
          logFile.close();
     }
...
void writeyearEnd()  //write --totals for month in milliseconds.
{
     ...
     else
     {
          ...
          yearData = "Year end elapsed time:  ";
          yearData += totalYear;
          yearData += " , ";
          yearData += dtStamp;
          ...
          if (parsed)
          {
               ...
               if (strcmp(action, "GET") == 0)
               {
                    ...
                    if ((strcmp(path, "/Furnace") == 0) || (strcmp(path, "/") == 0))   // Respond with the path that was accessed.
                    {
                         ...
                         client.print(":  ");
                         client.println(totalMonth, 4);
                         client.println(" <br><br><br>");

                         //readYear();

                         client.print("  Total Minutes for Year:  ");
                         client.println(totalYear, 4);
